...
impl ExecutedBlock {
	...
	fn new(state: State<StateDB>, last_hashes: Arc<LastHashes>, tracing: bool, storage_writing: bool) -> ExecutedBlock {
		ExecutedBlock {
...
impl<'x> OpenBlock<'x> {
	...
	pub fn new<'a>(
		...
		let mut r = OpenBlock {
			block: ExecutedBlock::new(state, last_hashes, tracing, storage_writing),
			engine: engine,
		};
...
mod tests {
	...
	fn enact_and_seal(
		...
		Ok(enact_bytes(block_bytes, engine, tracing, storage_writing, db, parent, last_hashes, factories)?
	...
	fn open_block() {
		...
		let b = OpenBlock::new(&*spec.engine, Default::default(), false, false, db, &genesis_header, last_hashes, Address::zero(), (3141562.into(), 31415620.into()), vec![], false, &mut Vec::new().into_iter()).unwrap();
	...
	fn enact_block() {
		...
		let db = spec.ensure_db_good(get_temp_state_db(), &Default::default()).unwrap();
		let last_hashes = Arc::new(vec![genesis_header.hash()]);
		let b = OpenBlock::new(engine, Default::default(), false, false, db, &genesis_header, last_hashes.clone(), Address::zero(), (3141562.into(), 31415620.into()), vec![], false, &mut Vec::new().into_iter()).unwrap()
			.close_and_lock().unwrap().seal(engine, vec![]).unwrap();
		let orig_bytes = b.rlp_bytes();
		let orig_db = b.drain().state.drop().1;

		let db = spec.ensure_db_good(get_temp_state_db(), &Default::default()).unwrap();
		let e = enact_and_seal(orig_bytes.clone(), engine, false, false, db, &genesis_header, last_hashes, Default::default()).unwrap();
	...
	fn enact_block_with_uncle() {
		...
		let db = spec.ensure_db_good(get_temp_state_db(), &Default::default()).unwrap();
		let last_hashes = Arc::new(vec![genesis_header.hash()]);
		let mut open_block = OpenBlock::new(engine, Default::default(), false, false, db, &genesis_header, last_hashes.clone(), Address::zero(), (3141562.into(), 31415620.into()), vec![], false, &mut Vec::new().into_iter()).unwrap();
