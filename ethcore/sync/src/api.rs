...
const COLLECT_BLOCKS_TIMER: TimerToken = 3;
const TX_TIMER: TimerToken = 4;
...
impl NetworkProtocolHandler for SyncProtocolHandler {
	fn initialize(&self, io: &NetworkContext) {
		if io.subprotocol_name() != WARP_SYNC_PROTOCOL_ID {
			...
			io.register_timer(COLLECT_BLOCKS_TIMER, Duration::from_millis(3000)).expect("Error registering collect blocks timer");
